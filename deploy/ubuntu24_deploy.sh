...
ASTELL_JWT_SECRET="${ASTELL_JWT_SECRET:-}"
...
ensure_auth_secret() {
  ...
  if [[ -z "${ASTELL_JWT_SECRET}" ]]; then
    ASTELL_JWT_SECRET="$(openssl rand -hex 32)"
    log "Generated ASTELL_JWT_SECRET for app sessions."
  fi
...
ASTELL_JWT_SECRET=${ASTELL_JWT_SECRET}
ASTELL_JWT_EXPIRE_MINUTES=${ASTELL_JWT_EXPIRE_MINUTES}
ASTELL_AUTH_BOOTSTRAP_MODE=${ASTELL_AUTH_BOOTSTRAP_MODE}
PYTHONUNBUFFERED=1
EOF
  chmod 0644 "${ENV_FILE}"
}
